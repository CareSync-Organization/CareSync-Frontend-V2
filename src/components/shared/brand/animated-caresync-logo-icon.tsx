...
import { motion as m, useAnimationFrame, useMotionValue, useReducedMotion, useTransform } from "framer-motion";
...
const BOTTOM_LOOP_PATH = "M64 140 C78 172 101 190 128 190 C155 190 178 172 192 140";
...
} satisfies Record<LogoState, { topY: number; bottomY: number; rotate: number }>;
...
function CareSyncLogoMark({
  ...
  const shouldAnimate = motion !== "none" && motion !== "spinning" && !prefersReducedMotion;
  const isDynamic = motion === "dynamic" && shouldAnimate;
  const state: LogoState = synced ? "synced" : shouldAnimate ? localState : "idle";
  ...
  const topOrbitCx = useTransform(spinAngle, (angle) => 128 + 88 * Math.cos(angle));
  const topOrbitCy = useTransform(spinAngle, (angle) => 128 + 88 * Math.sin(angle));
  const bottomOrbitCx = useTransform(spinAngle, (angle) => 128 + 88 * Math.cos(angle + Math.PI));
  const bottomOrbitCy = useTransform(spinAngle, (angle) => 128 + 88 * Math.sin(angle + Math.PI));
  ...
  return (
    <m.svg
      ...
      width={size}
      height={size}
      className={cn("block overflow-visible", interactive && !isSpinning && "cursor-pointer", className)}
      role="img"
    ...
    >
      <defs>
        <filter id={glowId} x="-36%" y="-36%" width="172%" height="172%" colorInterpolationFilters="sRGB">
          <feDropShadow dx="0" dy="10" stdDeviation="12" floodColor={palette.shadow} floodOpacity="0.78" />
          <feDropShadow dx="0" dy="2" stdDeviation="3" floodColor={palette.accent} floodOpacity="0.35" />
        </filter>

        <linearGradient id={gradientId} x1="54" y1="68" x2="202" y2="188" gradientUnits="userSpaceOnUse">
        ...
        <linearGradient id={orbitId} x1="58" y1="60" x2="198" y2="196" gradientUnits="userSpaceOnUse">
      ...
      <m.g
        ...
        transition={isDynamic ? { duration: 3.2, repeat: Infinity, ease: "easeInOut" } : spring}
      ...
      <m.g animate={{ y: loop.topY, rotate: loop.rotate }} transition={spring} style={{ transformOrigin: "128px 128px" }}>
        <path
          d={TOP_LOOP_PATH}
          fill="none"
      ...
      <m.g animate={{ y: loop.bottomY, rotate: -loop.rotate }} transition={spring} style={{ transformOrigin: "128px 128px" }}>
        <path
          d={BOTTOM_LOOP_PATH}
          fill="none"
          stroke={`url(#${gradientId})`}
      ...
      {isSpinning ? (
        <>
          ...
          <m.line
            ...
            opacity="0.72"
          />
          <m.circle cx={topOrbitCx} cy={topOrbitCy} r="8" fill={palette.primary} stroke={palette.accentSoft} strokeWidth="2" />
          <m.circle cx={bottomOrbitCx} cy={bottomOrbitCy} r="8" fill={palette.primary} stroke={palette.accentSoft} strokeWidth="2" />
          <m.circle
            cx={128}
            cy={128}
            r={10}
        ...
        <>
          <m.line
            x1="128"
            y1="128"
            animate={{ x2: nodes.top.cx, y2: nodes.top.cy, opacity: showTrails ? 1 : 0 }}
            initial={false}
            stroke={`url(#${trailRightId})`}
            strokeWidth="9"
            strokeLinecap="round"
          ...
          <m.line
            x1="128"
            y1="128"
            animate={{ x2: nodes.bottom.cx, y2: nodes.bottom.cy, opacity: showTrails ? 1 : 0 }}
            initial={false}
            stroke={`url(#${trailLeftId})`}
            strokeWidth="9"
            strokeLinecap="round"
            ...
            return (
              <m.circle
                ...
                animate={{
                  ...
                  cy: node.cy,
                  r: node.r,
                  scale: isDynamic && isCenter ? [1, 1.18, 1] : state === "synced" && isCenter ? [1, 1.16, 1] : 1,
                }}
                initial={false}
                ...
                transition={isDynamic && isCenter ? { duration: 1.8, repeat: Infinity, ease: "easeInOut" } : spring}
...
export function CareSyncLogoBadge({ className, size = 56, tone = "brand", motion = "subtle", ...props }: CareSyncLogoIconProps) {
  const palette = palettes[tone];
  ...
  return (
    <div
      className={cn(
        ...
        tone === "brand" ? "bg-white shadow-cyan-950/10 ring-black/5" : "bg-white/12 shadow-cyan-950/10 ring-white/20",
        className,
      )}
      style={{ width: size, height: size, color: palette.text }}
    >
      <CareSyncLogoMark size={Math.round(size * 0.76)} tone={tone} motion={motion} {...props} />
    </div>
  );
}
...
export function CareSyncLogoLockup({
  ...
  return (
    <div className={cn("flex items-center gap-3", className)} style={{ color: palette.text }}>
      <CareSyncLogoBadge size={size} tone={tone} motion={motion} {...props} />
      <div className="min-w-0">
        <p className="text-lg font-semibold text-white leading-none tracking-tight">CareSync</p>
        {tagline ? (
          <p className="mt-1 text-xs leading-none" style={{ color: palette.mutedText }}>
            {tagline}
          </p>
        ) : null}
      </div>
...
export function CareSyncLogoScene({ className, size = 220, tone = "light", motion = "dynamic", ...props }: CareSyncLogoProps) {
  const prefersReducedMotion = useReducedMotion();
...
export function CareSyncLogo({ variant = "icon", ...props }: CareSyncLogoProps) {
